...
export const Authentication = () => {
  ...
  const isNotUnlimited =
    user?.subscriptionTokens !== SUBSCRIPTION_UNLIMITED_TOKENS
  ...
  const availableSubscriptions = Array.isArray(stripeProducts)
    ? stripeProducts.filter((product) => {
        if (product.productType !== PRODUCT_TYPE_AI_PLUGIN) {
          return false // Only display subs
        }
        ...
        if (isMonthlySubscription) {
          return (
            ...
            (isNotUnlimited &&
              ...
              Number(product.tokens) === SUBSCRIPTION_UNLIMITED_TOKENS)
        ...
        if (isNotUnlimited)
          return (
            Number(product.tokens) === SUBSCRIPTION_UNLIMITED_TOKENS &&
            product.interval === 'year'
          ) // Show only unlimited plans for non unlimited plans
  ...
  const getTokenCountColor = (tokenCount: number) => {
    if (tokenCount > 30 || tokenCount == SUBSCRIPTION_UNLIMITED_TOKENS)
      return styles.tokenCountGreen
  ...
  const getProductHeadingStyle = (interval: string) => {
    if (interval === 'month') return styles.monthlyHeading
    else if (interval === 'year') return styles.yearlyHeading
    else return styles.freeHeading
  }
      ...
      {user ? (
            ...
            <div className={styles.subscriptionBox}>
              {validSubscription && (
                <>
                  ...
                  <p>
                    <strong>Type:</strong> {user.subscriptionName}
                  </p>
                  <p>
                    <strong>Interval:</strong> {user.subscriptionInterval}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {(user.subscriptionTokens ?? 0) >=
                    SUBSCRIPTION_UNLIMITED_TOKENS
                      ? 'Unlimited'
                      : user.subscriptionTokens}
                  </p>
              ...
              {availableSubscriptions.length > 0 && (
                <>
                  ...
                  <p>
                    *** An action is either a generated chat message or an
                    accepted code completion suggestion
                  </p>
                  {subscriptionInProgress.length > 0 ? (
                    ...
                    <div className={styles.subscriptionOptions}>
                      {availableSubscriptions.map((product) => (
                        ...
                        >
                          <h4
                            className={getProductHeadingStyle(product.interval)}
                          >
                            {product.name}
                          </h4>
                          <ul>
                          ...
                          <VSCodeButton
                            ...
                            disabled={
                              subscriptionInProgress.length > 0 ||
                              (user.subscriptionUsedFree == true &&
                                Number(product.price) === 0)
                            }
                          >
        ...
        <div>
          <p style={{ fontWeight: 'normal', color: 'red' }}>
            Note: Your dAppForge account is linked to the authentication
            provider you choose. If you sign up with both, you will have two
            different accounts.
          </p>
          <p style={{ fontWeight: 'normal', color: 'red' }}>
            Remember to go to the "dappforge language” section in settings and
            select the option you will be coding in (Substrate, ink!, Solidity,
            Rust, etc.) to make sure the LLM produces accurate results.
          </p>
          <VSCodeButton
            className={styles.authenticateButton}
