...
export class Stripe {
  ...
  private handleMessage = async (message: ClientMessage<string>) => {
    switch (message.type) {
      ...
      case AUTHENTICATION_EVENT_NAME.buyTopUp: {
        const { data: priceData } = message
        if (!priceData) {
          throw new Error('Price data is undefined')
        }
        const { priceId } = JSON.parse(priceData)
        if (priceId && priceId.length > 0) {
          return await this._handleTopUp(priceId)
        }
        throw new Error('PriceId is undefined')
      }
  ...
  private _handleSubscription = async (priceId: string, coupon: string) => {
    ...
    if (priceId === 'free') {
      ...
      if (
        ...
        await this._stripeCreateCheckoutSession(
          priceId,
          user,
          coupon,
          PRODUCT_TYPE_AI_PLUGIN
  ...
  private _handleTopUp = async (priceId: string) => {
    const config = workspace.getConfiguration('dappforge')
    const apiBaseUrl: string | undefined = config.get('apiUri')
    const user: User | null | undefined = getStoredUser()
    if (!user || !apiBaseUrl) return
    ...
    try {
      ...
      if (typeof response === 'string') {
        if (response.length > 0) {
          ...
          const jsonResponse: {
            status: number
            response: { error?: string; discount?: number }
          } = JSON.parse(response)
    ...
    try {
      ...
      if (typeof response === 'string') {
        if (response.length > 0) {
          ...
          const jsonResponse: {
            status: number
            response: { error?: string; cancelResult?: string }
          } = JSON.parse(response)
  ...
  private _stripeUpdateSubscription = async (
    priceId: string,
    user: User,
    coupon: string
  ) => {
    ...
    try {
      ...
      if (typeof response === 'string') {
        if (response.length > 0) {
          ...
          const jsonResponse: {
            status: number
            response: { error?: string; updateResult?: string }
          } = JSON.parse(response)
          if (jsonResponse.status != 200) {
            throw Error(jsonResponse.response.error)
          }
  ...
  private _stripeCreateCheckoutSession = async (
    priceId: string,
    user: User,
    coupon: string,
    productType: string
        ...
        if (response.length > 0) {
          ...
          if (jsonResponse.status != 200) {
            ...
            if (
              ...
              jsonResponse.response.subscriptionFound
            ) {
              ...
              window.showInformationMessage(
                'Found an existing active subscription.'
              )
            ...
            jsonResponse.response.url &&
            jsonResponse.response.url.length > 0
          ) {
  ...
  private _processFreeSubscription = async (
    priceId: string,
    user: User,
    apiBaseUrl: string
  ) => {
  ...
  private _processWebhookMessage = async (message: unknown) => {
    try {
      ...
      logger.log(`_processWebhookMessage message: ${JSON.stringify(message)}`)
      ...
      logger.log(`_processWebhookMessage msg: ${JSON.stringify(msg)} `)
      ...
      // Normalize response:
      // - If response is a JSON-looking string, parse it.
      // - If response is a plain string (e.g., "Connection: ..."), treat as info and ignore for routing.
      // - If response is already an object, use as-is.
      let resp:
        | {
            subscriptionUpdated?: boolean
            apiTokensUpdated?: boolean
      ...
      if (typeof msg.response === 'string') {
        const trimmed = msg.response.trim()
        const looksJson =
        ...
        if (looksJson) {
          ...
          logger.log(`_processWebhookMessage info: ${trimmed}`)
          resp = undefined
        }
      ...
      logger.log(
        `_processWebhookMessage normalized response: ${
          resp ? JSON.stringify(resp) : '<non-json/none>'
        }`
      )

      // After the normalized-response logger

      // If we didn't get a JSON object, there's nothing to route. Exit early.
      if (!resp || typeof resp !== 'object') {
        logger.log('Received a message from the API with no response')
        return
      }

      // Narrow types after the guard
      const productType = (resp as any).productType as string | undefined
      const subscriptionUpdated = (resp as any).subscriptionUpdated as
        | boolean
        | undefined
      const apiTokensUpdated = (resp as any).apiTokensUpdated as
        | boolean
        | undefined

      logger.log(
        `_processWebhookMessage productType: ${productType} subscriptionUpdated: ${subscriptionUpdated} apiTokensUpdated: ${apiTokensUpdated} `
      )

      // 1) API token top-ups (AI API)
      if (productType === PRODUCT_TYPE_AI_API && apiTokensUpdated) {
        logger.log('Received a message from the API to update API tokens')
        await checkAuthenticationStatus(() => {
          this._webviewView.webview.postMessage({
            type: AUTHENTICATION_EVENT_NAME.buyTopUp,
            value: { data: getStoredUser() }
          })
        })
        return
      }

      // 2) Plugin subscription updates (anything that's not an AI API top-up)
      // Treat missing productType as a plugin subscription event.
      if (subscriptionUpdated && productType === PRODUCT_TYPE_AI_PLUGIN) {
        logger.log('Received a message to update plugin subscription')
        await checkAuthenticationStatus(() => {
          this._webviewView.webview.postMessage({
            type: AUTHENTICATION_EVENT_NAME.subscribe,
            value: { data: '' }
  ...
  private _setupStripeWebsocket = async (
    ...
    if (requestType == dAppForgeRequestTypes.stripe) {
      ...
      if (
        !this._stripeWebhookWebSocket ||
        !this._stripeWebhookWebSocket.isSocketOpen()
      ) {
        this._stripeWebhookWebSocket = new WebSocketHandler(
          url,
          this._processWebhookMessage
        )
        newSocket = true
      }
