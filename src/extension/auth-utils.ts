  ...
  if (isAuthenticated()) {
    ...
    if (user !== null && user !== undefined) {
      if (!user.subscriptionId || user.subscriptionId.length <= 0)
        ...
        user.tokenCount != SUBSCRIPTION_UNLIMITED_TOKENS &&
        (!user.tokenCount || user.tokenCount <= 0)
      )
...
export function getWebSocketUri(
  ...
  if (context)
    uri = context.globalState.get<string>(
      `${WEBSOCKET_URI_STORAGE_KEY}-${requestType}`
    )
  return uri
}

function setWebSocketUri(
  requestType: string,
...
export const authenticate = async (authType: string, fn?: () => void) => {
  ...
  app.get('/auth/:id/:accessToken/:apiPassword/:authType', async (req, res) => {
    ...
    res.end(
      '<h1>dAppForge authentication was successful, you can close this now</h1>'
    )
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (maybeJson && typeof maybeJson === 'object') {
          errorMsg =
            (maybeJson.error?.message as string) ||
            (maybeJson.message as string) ||
            errorText
        }
      } catch {
        // ignore
      ...
      throw new Error(
        `dAppForge: Unable to update email, status: ${
          response.status
...
export const processGenerateApiKey = async (fn?: () => void) => {
  const config = workspace.getConfiguration('dappforge')
  const apiBaseUrl: string | undefined = config.get('apiUri')
  const user: User | null | undefined = getStoredUser()
  if (!user || !apiBaseUrl) return
  ...
  let error = ''
  const url = `${apiBaseUrl}/auth/generate-api-key/${user.id}`

  try {
    logger.log(`~~~~~> processGenerateApiKey url: ${url}`)
    const fetchOptions = {
      method: 'GET',
      headers: {
        authorization: `Basic ${getBasicAuthToken(user)}`,
    ...
    const response = await fetch(url, fetchOptions)
    logger.log(`<~~~~~ processGenerateApiKey status: ${response.status}`)
    if (!response.ok) {
      throw Error('dAppForge: Unable to generate an API key')
    } else {
      const user: User | null | undefined = getStoredUser()
      if (user) {
        ...
        /*logger.log(
          `<~~~~~ processGenerateApiKey json response: ${JSON.stringify(
            data,
            undefined,
            2
          )}`
        ...
        if (data) {
          user.apiKeyHash = data.apiKeyHash
          user.apiMaskedKey = data.apiMaskedKey
          user.apiRawKey = data.apiRawKey
          setStoredUser(user)
        }
      }
    }
    ...
    if (e instanceof Error) {
      ...
      error = `-----> processGenerateApiKey: error: ${e}`
      logger.log(error)
    }
    window.showErrorMessage(error)
  } finally {
  ...
  if (!user) {
    ...
    try {
      ...
      if (user.authType == 'google')
        url = `${apiBaseUrl}/auth/google_check/${user.id}`
      ...
      const data = JSON.parse(txt)
      logger.log(
        `<~~~~~ checkAuthenticationStatus json response: ${JSON.stringify(
          data,
          undefined,
          2
        )}`
      ...
      if (
        ...
        user = {
          ...
          apiMaskedKey: userData.apiMaskedKey,
          apiRawKey: userData.apiRawKey,
          apiKeyHash: userData.apiKeyHash,
          apiFreeTokenCap: userData.apiFreeTokenCap,
          apiFreeTokenBalance: userData.apiFreeTokenBalance,
          apiPaidTokenBalance: userData.apiPaidTokenBalance,
          apiUsedInputTokensMonth: userData.apiUsedInputTokensMonth,
          apiUsedOutputTokensMonth: userData.apiUsedOutputTokensMonth,
          apiFreeResetDate: userData.apiFreeResetDate,
          apiLastUsed: userData.apiLastUsed
        }
        setStoredUser(user)
      } else {
      ...
      if (
        data &&
        Object.prototype.hasOwnProperty.call(data, 'completionWebSocketUri') &&
        data['completionWebSocketUri'].length > 0 &&
        Object.prototype.hasOwnProperty.call(data, 'chatWebSocketUri') &&
        data['chatWebSocketUri'].length > 0 &&
        Object.prototype.hasOwnProperty.call(data, 'aiApiWebSocketUri') &&
        data['aiApiWebSocketUri'].length > 0 &&
        Object.prototype.hasOwnProperty.call(data, 'tokenCountWebSocketUri') &&
        data['tokenCountWebSocketUri'].length > 0 &&
        Object.prototype.hasOwnProperty.call(data, 'stripeWebSocketUri') &&
        data['stripeWebSocketUri'].length > 0 &&
        Object.prototype.hasOwnProperty.call(
          data,
          'stripeWebhookWebSocketUri'
        ) &&
        data['stripeWebhookWebSocketUri'].length > 0
      ) {
        //if (getWebSocketUri() !== data['websocketuri']) {
        setWebSocketUri(
          dAppForgeRequestTypes.autocompletion,
          data['completionWebSocketUri']
        )
        updateProviders(
          dAppForgeRequestTypes.autocompletion,
          data['completionWebSocketUri']
        ...
        setWebSocketUri(
          ...
          data['tokenCountWebSocketUri']
        )
        setWebSocketUri(
          dAppForgeRequestTypes.feedback,
          data['aiApiWebSocketUri']
        )
        setWebSocketUri(
          dAppForgeRequestTypes.stripe,
          data['stripeWebSocketUri']
        )
        setWebSocketUri(
          dAppForgeRequestTypes.stripeWebhook,
          data['stripeWebhookWebSocketUri']
        )
        if (
          Object.prototype.hasOwnProperty.call(data, 'stripeProducts') &&
          data['stripeProducts'].length > 0
        ) {
